...
ClassImp(FairDbRunInfo);

#include "FairDbReader.tpl"
template class  FairDbReader<FairDbRunInfo>;

#include "FairDbWriter.tpl"
template class  FairDbWriter<FairDbRunInfo>;



FairDbRunInfo::FairDbRunInfo(const char* name, const char* title, const char* context, Bool_t own)
...
void FairDbRunInfo::putParams(FairParamList* list)
{
  std::cout<<"-I- FairDbRunInfo::putParams() called"<<std::endl;
  if(!list) { return; }
...
Bool_t FairDbRunInfo::getParams(FairParamList* list)
{
...
void FairDbRunInfo::clear()
{
  fCompId=fRunId=0;
}


string FairDbRunInfo::GetTableDefinition(const char* Name)
{

  string sql("create table ");
  if ( Name ) { sql += Name; }
  else { sql += "FairDbRUNINFO"; }
...
void FairDbRunInfo::Fill(FairDbResultPool& res_in,
                        const FairDbValRecord* valrec)
{
  res_in >> fCompId  >> fRunId  >> fRunTime;
}

void FairDbRunInfo::Store(FairDbOutTableBuffer& res_out,
...
void FairDbRunInfo::fill(UInt_t rid)
{
  ...
  for (int i = 0; i < numRows; ++i) {
    FairDbRunInfo* cgd = (FairDbRunInfo*) fParam_Reader->GetRow(i);
    if (!cgd) { continue; }
...
void FairDbRunInfo::store(UInt_t rid)
{
  ...
  if ( ! stmtDbn.get() ) {
    cout << "-E-  FairDbRunInfo::Store()  Cannot create statement for Database_id: " << GetDbEntry()
  ...
  if (! fMultConn->GetConnection(GetDbEntry())->TableExists("FairDbRUNINFO") ) {
    sql_cmds.push_back(FairDb::GetValDefinition("FairDbRUNINFO").Data());
    sql_cmds.push_back(FairDbRunInfo::GetTableDefinition());
  }
  ...
  while( itr != itrEnd ) {
    ...
    if ( stmtDbn->PrintExceptions() ) {
      fail = true;
      cout << "-E- FairDbRunInfo::Store() ******* Error Executing SQL commands ***********  " << endl;
    }
  ...
  *fParam_Writer<< (*this);
  ...
  if ( !fParam_Writer->Close() ) {
    fail = true;
    cout << "-E- FairDbRunInfo::Store() ******** Cannot do IO on class: " << GetName() <<  endl;
  }

}

void FairDbRunInfo::Print()
{
  std::cout<<"   FairDbRunInfo Parameters: "<<std::endl;
  std::cout<<"   fCompId: "<<  fCompId <<  std::endl;

}


Bool_t FairDbRunInfo::Compare(const FairDbRunInfo& that ) const {
  //  
...
FairDbWriter<FairDbRunInfo>* FairDbRunInfo::ActivateWriter(Int_t rid)
{
...
Int_t FairDbRunInfo::CalcUniqueSeqNo(Int_t run)
{
...
Bool_t FairDbRunInfo::Commit()
{
  ...
  // Check if for this DB entry the table already exists.
  // If not call the corresponding Table Definition Function
  std::string tbl  = "FairDbRUNINFO";
  std::string tblv = "FairDbRUNINFOVAL";;
  ...
  while( itr != itrEnd ) {
	...
	if ( stmtDbn->PrintExceptions() ) {
	  fail = kTRUE;
	  cout << "-E- FairDbRunInfo::Insert() ******* Error Executing SQL commands ***********  " << endl;
	}
