...
ClassImp(CbmStsDbQaIvPar);


static FairDbParRegistry<CbmStsDbQaIvPar> qa_iv("CbmStsDbQaIvPar");

#include "FairDbReader.tpl"
template class  FairDbReader<CbmStsDbQaIvPar>;

#include "FairDbWriter.tpl"
template class  FairDbWriter<CbmStsDbQaIvPar>;



CbmStsDbQaIvPar::CbmStsDbQaIvPar(const char* name, const char* title, const char* context, Bool_t own)
...
CbmStsDbQaIvPar::~CbmStsDbQaIvPar()
{
...
void CbmStsDbQaIvPar::putParams(FairParamList* list)
{
  std::cout<<"-I- CbmStsDbQaIvPar::putParams() called"<<std::endl;
  if(!list) { return; }
...
Bool_t CbmStsDbQaIvPar::getParams(FairParamList* list)
{
...
string CbmStsDbQaIvPar::GetTableDefinition(const char* Name)
{
  string sql("create table ");
  if ( Name ) { sql += Name; }
  else { sql += "CBMSTSDBQAIVPAR"; }
...
void CbmStsDbQaIvPar::Fill(FairDbResultPool& res_in,
...
void CbmStsDbQaIvPar::Store(FairDbOutTableBuffer& res_out,
                         const FairDbValRecord* valrec) const
{
  // Check first if arrays are defined
  if (!fI || !fV ) {
  	cout << "-E- CbmStsDbQaIvPar::Store() SQL I/O  : Array Structure not defined " << endl;
    exit(0); 
  }
...
void CbmStsDbQaIvPar::fill(UInt_t rid)
{
  ...
  for (int i = 0; i < numRows; ++i) {
    CbmStsDbQaIvPar* cgd = (CbmStsDbQaIvPar*) fParam_Reader->GetRow(i);
    if (!cgd) { continue; }
...
void CbmStsDbQaIvPar::store(UInt_t rid)
{
  ...
  if ( ! stmtDbn.get() ) {
    cout << "-E-  CbmStsDbQaIvPar::Store()  Cannot create statement for Database_id: " << GetDbEntry()
  ...
  if (! fMultConn->GetConnection(GetDbEntry())->TableExists("CBMSTSDBQAIVPAR") ) {
    sql_cmds.push_back(FairDb::GetValDefinition("CBMSTSDBQAIVPAR").Data());
    sql_cmds.push_back(CbmStsDbQaIvPar::GetTableDefinition());
  }
  ...
  while( itr != itrEnd ) {
    ...
    if ( stmtDbn->PrintExceptions() ) {
      fail = true;
      cout << "-E- CbmStsDbQaIvPar::Store() ******* Error Executing SQL commands ***********  " << endl;
    }
  ...
  *fParam_Writer<< (*this);
  ...
  if ( !fParam_Writer->Close() ) {
    fail = true;
    cout << "-E- CbmStsDbQaIvPar::Store() ******** Cannot do IO on class: " << GetName() <<  endl;
  }

}

void CbmStsDbQaIvPar::Print()
{
...
void CbmStsDbQaIvPar::SetN(Int_t nele){
  fN=nele;
  if(!fI) fI = new Double_t[fN];
  if(!fV) fV = new Double_t[fN];
}  

Bool_t CbmStsDbQaIvPar::Compare(const CbmStsDbQaIvPar& that ) const {
...
FairDbWriter<CbmStsDbQaIvPar>* CbmStsDbQaIvPar::ActivateWriter(Int_t rid)
{
